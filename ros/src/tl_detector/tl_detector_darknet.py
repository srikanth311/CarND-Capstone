...
class TLDetectorDarkNet(object):
    def __init__(self):
    ...
    def prcb(msg):
        ...
        self.classes = sorted_msg[0].id
        self.scores = sorted_msg[0].probability
    ...
    def interpret_classification(self):
        if self.scores is None or self.scores <= 0.2:
            return TrafficLight.UNKNOWN
        else:
            if ( (int(self.classes) == 1) or (int(self.classes) == 2) or (int(self.classes) == 3) ):
                return TrafficLight.RED
            elif ( (int(self.classes) == 4) or (int(self.classes) == 5) or (int(self.classes) == 6) ):
                return TrafficLight.GREEN
            elif ( (int(self.classes) == 7) ):
    ...
    def waypoints_cb(self, waypoints):
        ...
        wp_2d = [[waypoint.pose.pose.position.x, waypoint.pose.pose.position.y] for waypoint in waypoints.waypoints]
        self.waypoint_tree = KDTree(wp_2d)
...
def get_light_state(self, light):
    ...
    def process_traffic_lights(self):
        ...
        if(self.pose and self.waypoint_tree):
            ...
            # find the closest visible traffic light, if there is one
